...
async function publicar() {
  ...
  try {
    ...
    const blob = await upload(`noticias/${slugArchivo}/${archivo.name}`, archivo, {
      access: "public",
    ...
    exitoMensaje.value = "La noticia fue publicada correctamente."
    ...
    await navigateTo(`/noticias/${noticia.slug}`)
  } catch (error) {
    errorMensaje.value = mensajeDeError(error)
  } finally {
    enviando.value = false
    ...
    <main class="panel container">
      ...
      <section v-else-if="!reporter" class="auth-card">
        ...
        <form @submit.prevent="ingresar">
          ...
          <label>
            Contraseña
            <input v-model="credenciales.password" type="password" autocomplete="current-password" minlength="8" required>
          </label>
      ...
      <section v-else class="publisher">
        ...
        <form class="publisher-form" @submit.prevent="publicar">
          ...
          <label class="field-wide">
            Contenido
            <textarea v-model.trim="formulario.contenido" rows="12" minlength="30" maxlength="20000" required></textarea>
          </label>
